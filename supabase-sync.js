...
const supabaseSync = (() => {
  ...
  const syncIntervalMs = Math.max(500, Number(config.syncIntervalMs) || 1000);
  const eventDebounceMs = Math.max(50, Number(config.eventDebounceMs) || 120);
  const fullSnapshotIntervalMs = Math.max(5000, Number(config.fullSnapshotIntervalMs) || 10000);
