...
/* ============================================================================
   FLOWSIM FINANCIERO · SINCRONIZACIÓN OPTIMIZADA CON SUPABASE V4
   - Conserva todos los datos operativos y financieros sin duplicarlos.
   - El recorrido se guarda una sola vez en eventos; no se repite en pasos/JSON.
   - El modelo y la configuración se guardan una vez por ejecución.
...
const supabaseSync = (() => {
  ...
  const syncIntervalMs = Math.max(1000, Number(config.syncIntervalMs) || 2000);
  const eventDebounceMs = Math.max(250, Number(config.eventDebounceMs) || 500);
  const fullSnapshotIntervalMs = Math.max(60000, Number(config.fullSnapshotIntervalMs) || 300000);
  const metricIntervalMs = Math.max(10000, Number(config.metricIntervalMs) || 30000);
  const maxEventsPerSync = Math.max(250, Number(config.maxEventsPerSync) || 1500);
  const requestTimeoutMs = Math.max(5000, Number(config.requestTimeoutMs) || 30000);

  const SESSION_KEY = "flowsimSupabaseSessionV4";
  const OUTBOX_DB = "flowsimSupabaseOutboxV4";
  const OUTBOX_STORE = "outbox";
  const OUTBOX_FALLBACK_PREFIX = "flowsimSupabaseOutboxV4:";
  ...
  function compactPayloadForStorage(payload, complete) {
    const compact = payload;

    // La tabla de eventos ya conserva el recorrido; se evita duplicarlo.
    compact.pasos = [];
    compact.variables = [];
    compact.variables_cambiadas = [];
    compact.snapshot = null;
    compact.nodos = [];
    compact.conexiones = [];
    compact.runtime_nodos = [];

    // Los escenarios comparativos son pocos y solo se envían en respaldos completos.
    if (!complete) compact.escenarios = [];

    compact.personas = (compact.personas || []).map(row => {
      const clean = { ...row };
      delete clean.ruta_json;
      delete clean.pasos;
      delete clean.linea_tiempo;
      delete clean.datos_adicionales;
      return clean;
    });

    compact.eventos = (compact.eventos || []).map(row => {
      const clean = { ...row };
      delete clean.datos_adicionales;
      return clean;
    });

    compact.movimientos = (compact.movimientos || []).map(row => {
      const clean = { ...row };
      delete clean.datos_adicionales;
      return clean;
    });

    compact.creditos = (compact.creditos || []).map(row => {
      const clean = { ...row };
      delete clean.datos_adicionales;
      return clean;
    });

    compact.cartera = (compact.cartera || []).map(row => {
      const clean = { ...row };
      delete clean.datos_adicionales;
      return clean;
    });

    compact.escenarios = (compact.escenarios || []).map(row => {
      const clean = { ...row };
      delete clean.datos_adicionales;
      return clean;
    });

    compact.metricas = (compact.metricas || []).map(row => {
      const clean = { ...row };
      delete clean.historial_metricas;
      delete clean.metricas;
      delete clean.estado_financiero;
      delete clean.runtime_nodos;
      delete clean.datos_adicionales;
      return clean;
    });

    if (compact.simulacion) {
      // Nunca se almacenan estados visuales transitorios ni entidades activas.
      delete compact.simulacion.entidades_activas;
      delete compact.simulacion.estado_runtime;
      delete compact.simulacion.estado_completo;
      delete compact.simulacion.evento_operativo;

      // En sincronizaciones normales solo se envían columnas escalares.
      if (!complete) {
        delete compact.simulacion.escenario;
        delete compact.simulacion.estado_financiero;
        delete compact.simulacion.metricas;
        delete compact.simulacion.modelo;
        delete compact.simulacion.definiciones;
        delete compact.simulacion.escenarios_guardados;
      }
    }
  ...
  function buildPayload(forceFull = false) {
    ...
    // El estado visual completo y el inventario genérico de variables no se
    // reconstruyen: sus datos analíticos ya están normalizados en tablas.
    const completeState = {};
    ...
    const metricDue = complete || now - lastMetricSavedAt >= metricIntervalMs;
    const metricRows = metricDue ? buildMetricRows(snapshotKey) : [];
    if (metricRows.length) {
      commits.metricKey = metricSignature;
      commits.metricSavedAt = now;
    }
    ...
    const payload = {
      ...
      escenarios: complete ? buildScenarioRows() : [],
      metricas: metricRows
    };

    return { payload: compactPayloadForStorage(payload, complete), commits, complete };
  }
  ...
  function applyCommits(commits) {
    ...
    if (commits.metricSavedAt) lastMetricSavedAt = commits.metricSavedAt;
  }
  ...
  async function syncNow(forceFull = false, keepalive = false) {
    if (suspended || !runId || !isConfigured()) return false;
    if (syncing) {
      syncRequested = true;
      return false;
    }
  ...
  function beginRun() {
    ...
    // Cada ejecución obtiene un UUID nuevo. Si quedó una sesión finalizada en el
    // navegador, se descarta para impedir que una corrida anterior se reenvíe.
    const previousRunId = runId;
    clearSession();
    if (previousRunId) void deleteOutbox(previousRunId);
  ...
  async function finishRun(status = "finalizada") {
    ...
    const saved = await syncNow(true, true);

    // Solo se elimina la sesión local después de que Supabase confirmó el lote
    // final. Así una recarga no vuelve a mezclar la ejecución anterior.
    if (saved) {
      clearSession();
      await deleteOutbox(finishedRunId);
      runId = "";
      startedAt = "";
      endedAt = null;
      runStatus = "sin_iniciar";
      active = false;
      clearRunCaches();
      setConnectionStatus("Supabase: ejecución guardada", "connected");
    }
    return saved;
  }

  async function suspendForDeletion() {
    suspended = true;
    syncRequested = false;
    window.clearTimeout(requestSync.debounceId);

    const waitStartedAt = Date.now();
    while (syncing && Date.now() - waitStartedAt < requestTimeoutMs + 2000) {
      await new Promise(resolve => window.setTimeout(resolve, 100));
    }

    setConnectionStatus("Supabase: borrado preparado", "syncing");
    return true;
  }

  function resumeAfterDeletionFailure() {
    suspended = false;
    setConnectionStatus("Supabase: listo", "connected");
    return true;
  }

  async function discardAllLocalData() {
    suspended = true;
    syncRequested = false;
    window.clearTimeout(requestSync.debounceId);

    // Espera a que termine una solicitud que ya estaba en curso.
    const waitStartedAt = Date.now();
    while (syncing && Date.now() - waitStartedAt < requestTimeoutMs + 2000) {
      await new Promise(resolve => window.setTimeout(resolve, 100));
    }
    ...
    try {
      ...
      for (let index = 0; index < window.localStorage.length; index += 1) {
        ...
        if (
          ...
          (
            key === SESSION_KEY ||
            key.startsWith(OUTBOX_FALLBACK_PREFIX) ||
            key.startsWith("flowsimSupabaseOutbox")
          )
        ) {
          keys.push(key);
        }
    ...
    active = false;
    clearRunCaches();
    setConnectionStatus("Supabase: datos locales limpiados", "connected");
    return true;
  }
  ...
  function initialize() {
    ...
    // Una sesión ya terminada no debe revivir al volver a abrir la página.
    if (runId && !active && ["finalizada", "detenida", "reiniciada"].includes(runStatus)) {
      const staleRunId = runId;
      clearSession();
      void deleteOutbox(staleRunId);
      runId = "";
      startedAt = "";
      endedAt = null;
      runStatus = "sin_iniciar";
